...
각 문서는 단독으로 읽을 수 있도록 작성되어 있다. 웹 공격 시리즈는 HTTP 요청 구조 → 공격 분류 → WAF 룰 설계 순으로, AI 보안 시리즈는 LLM 기본기 → 공격 지형도 → 공격 유형 → 방어 전략 → 항목별 레시피 → 에이전트 실전 → CLI 보안 → 종합 랩 순으로 읽으면 흐름을 따라갈 수 있다.
