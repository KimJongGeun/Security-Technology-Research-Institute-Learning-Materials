...
- 영어 기준 약 4자 = 1토큰, 한국어는 1자 = 1-2토큰. 이 차이로 한국어가 영어보다 같은 작업에서 토큰을 2-3배 먹는다.
- 보안 관점: **rate limit, 비용, 컨텍스트 한계**가 모두 토큰 단위. DoS·비용 폭탄 공격은 토큰 남용으로 실현된다.

**컨텍스트 윈도우 (Context Window)**
- 한 번의 추론에서 모델이 한 번에 "볼 수 있는" 토큰 수. 공식 Anthropic 문서(2026-04 기준):
  - Claude Opus 4.7 — **1M 토큰**, 최대 출력 128K
  - Claude Sonnet 4.6 — **1M 토큰**, 최대 출력 64K
  - Claude Haiku 4.5 — **200K 토큰**, 최대 출력 64K
- 컨텍스트를 넘기면 **앞부분이 잘리거나, 긴 문서에서 중간 내용을 놓치는 "lost in the middle"** 현상 발생.
- 보안 관점: 공격자가 악성 지시를 **긴 문서 중간에 숨기는** 패턴(Indirect Prompt Injection)이 여기서 성립. 1M 컨텍스트에서는 숨길 공간이 훨씬 넓어진다.
...
- 보안 관점: 한 번의 명령이 **수십에서 수백 번의 도구 호출**로 확장된다. 각 호출이 감사 가능해야 하고, 누적 권한이 **원래 사용자 권한을 넘지 않아야** 한다 (principle of least privilege).
...
### 2.1 현재 모델 라인업 (2026-04 공식 기준)

| 모델 | 모델 ID | 컨텍스트 | 최대 출력 | 가격 (input/output per MTok) | 주 용도 |
|---|---|---|---|---|---|
| Claude Opus 4.7 | `claude-opus-4-7` | 1M | 128K | $5 / $25 | 복잡 에이전트·보안 분석 |
| Claude Sonnet 4.6 | `claude-sonnet-4-6` | 1M | 64K | $3 / $15 | 일반 챗·API·RAG 대부분 |
| Claude Haiku 4.5 | `claude-haiku-4-5-20251001` | 200K | 64K | $1 / $5 | 분류·라우팅·로그 전처리 |

※ Claude Mythos Preview는 Project Glasswing 초청 전용으로 일반 API에서 사용 불가.
...
관찰 포인트(결과는 모델 버전·프롬프트 상태에 따라 달라질 수 있음):
- 위와 같은 단순한 직접 인젝션은 Claude가 거부하는 경우가 많다. 그러나 **항상 거부한다는 보장은 없다** — 학습 가이드라인의 확률적 경향이지 강제 경계가 아니기 때문.
- **문구 난독화, 다국어, base64 인코딩, 롤플레이 등으로 바꾸면 우회율이 올라간다**는 것이 공개 벤치마크·레드팀 연구에서 반복 확인된 경향이다.
