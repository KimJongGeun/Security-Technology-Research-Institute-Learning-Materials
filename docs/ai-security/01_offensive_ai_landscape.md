...
### 1.3 과장을 걸러낸 실체
- 현재 공개된 발견 다수는 **역사적 버그**(수년에서 수십년 묵은)다. 최신 하드닝(ASLR, CFI, 메모리 태깅)이 적용된 환경에서의 신규 제로데이 발견률은 별개의 문제
...
```mermaid
xychart-beta
    title "Cybench 무가이드 풀이율 (2024→2025)"
    x-axis [2024, 2025]
    y-axis "풀이율 (%)" 0 --> 100
    bar [15, 93]
```

※ 2026-04는 "풀이율"이 아닌 "대회 순위(상위 6%)"로 다른 지표라 동일 축에 표시하지 않음.
...
### 3.1 잘하는 것
- **알려진 취약점 대량 스캐닝 + 체인 구성**: 인간 레드팀이 며칠 걸릴 조합을 수분에서 수시간에
...
```mermaid
flowchart LR
    subgraph 강점[공격 AI가 잘하는 영역]
        S1[레거시 CVE<br/>체이닝·스캐닝]
        S2[코드 정적 분석]
        S3[피싱·소셜<br/>엔지니어링 생성]
        S4[OSINT 프로파일링]
    end
    subgraph 약점[아직 제한적인 영역]
        W1[현대 하드닝 환경<br/>신규 제로데이]
        W2[물리·사이드채널]
        W3[타겟 고유<br/>비즈니스 로직]
        W4[장시간 네트워크<br/>운영 감각]
    end

    style 강점 fill:#ffe0e0
    style 약점 fill:#e0f0e0
```

※ 이 분류는 3.1·3.2의 정성적 기술을 시각화한 것이며, 각 항목의 정량 순위는 공개 벤치마크로 검증되지 않았다.
