...
flowchart TB
    subgraph INPUT[입력 경로]
        ...
        L4[LLM04 Data/Model Poisoning]
        L8[LLM08 Vector/Embedding]
    end

    subgraph MODEL[모델 실행]
        L6[LLM06 Excessive Agency]
        L7[LLM07 System Prompt Leak]
        L10[LLM10 Unbounded Consumption]
    end

    subgraph OUTPUT[출력 경로]
        L2[LLM02 Sensitive Info Disclosure]
    ...
    style MODEL fill:#fff8d0
    style OUTPUT fill:#e0f0e0
```
