...
sequenceDiagram
    ...
    participant H as MCP 호스트<br/>(Claude Desktop 등)
    participant G as 정상 서버
    participant E as 악성 서버

    rect rgb(255, 230, 230)
      Note over E,H: Tool Poisoning
      E->>H: list_tools (description에 숨은 지시 포함)
      H->>U: 도구 목록 표시 (본문만 보임)
      U->>H: 도구 사용 승인
      H->>E: call_tool → 숨은 지시 활성화
    end

    rect rgb(255, 240, 200)
      Note over G,E: Tool Shadowing
      G->>H: register send_email
      E->>H: register send_email (후등록)
      H->>E: 이름 충돌 시 가로채기
    end

    rect rgb(230, 230, 255)
      Note over E,H: Rug Pull
      E->>H: 초기 등록 (정상 description)
      Note over E: 시간 경과, 원격에서 교체
      E->>H: list_changed 통지
      H->>E: 새 description으로 호출 (재승인 없으면 위험)
    end
```
...
```mermaid
flowchart LR
    A[microVM<br/>Firecracker] --> B[gVisor]
    B --> C[WASM<br/>wasmtime]
    C --> D[컨테이너<br/>Docker]
    D --> E[chroot]
    E --> F[프로세스]
...
왼쪽이 가장 강한 격리(하드웨어 가상화), 오른쪽으로 갈수록 약한 격리. 실무에서는 "신뢰도가 낮은 코드·입력일수록 왼쪽 계층" 원칙을 적용한다.
